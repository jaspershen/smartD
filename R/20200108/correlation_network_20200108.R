...
library(tidyverse)
library(plyr)
setwd("data_analysis20200108/correlation_network/")
rm(list = ls())
...
cross_table <- cross_metabolite_table
...
cross_cor <-
cross_cor %>% 
  filter(P.adjust < 0.01 & abs(Correlation) > 0.6)
...
node_attr$sort <- 
  case_when(
    ...
    node_attr$node_class == "Phenylpropanoids and polyketides" ~ 10,
    node_attr$node_class == "Other" ~ 11
    )
