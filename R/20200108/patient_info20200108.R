...
(
  plot1 <- 
    temp_data %>% 
    ggplot(aes(GA2, factor(Patient_ID,  level = unique(Patient_ID)))) +
    ...
    geom_rect(aes(xmin = 40, ymin = 0, 
              ...
              fill = "grey", alpha = 0.01, 
              inherit.aes = FALSE) +
    geom_hline(yintercept = match(preterm7, unique(temp_data$Patient_ID)),
               colour = "#800000FF", linetype = 1) +
    geom_hline(yintercept = match(preterm21, unique(temp_data$Patient_ID)),
...
plot1 <- 
plot1 +
  ggplot2::annotate(geom = "point", shape = 17, 
                    colour = "black", size = 2,
...
info <- 
  info %>% 
  filter(ID %in% temp_data$Patient_ID)
