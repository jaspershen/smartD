###20191015 use all the identified metabolites (Level 1 and Level 2)
setwd("data_analysis20191015/correlation_network/identification_table/")
...
load("POS/smartd_rplc_pos_5")
ms1_data_pos <- smartd_rplc_pos_5@ms1.data[[1]]

load("NEG/smartd_rplc_neg_5")
ms1_data_neg <- smartd_rplc_neg_5@ms1.data[[1]]
...
smartd_rplc_pos <- identification_table_pos
smartd_rplc_neg <- identification_table_neg

smartd_rplc <- full_join(smartd_rplc_pos,
                         smartd_rplc_neg,
                         by = intersect(colnames(smartd_rplc_pos),
                                        colnames(smartd_rplc_neg)))
...
write.csv(smartd_rplc, "smartd_rplc.csv", row.names = FALSE)
...
colnames(cross_temp)[c(3, 4, 5, 338, 339, 340, 341, 342, 343, 344, 345)]
...
cross_temp <- 
  cross_temp %>% 
  select(., -c(colnames(cross_temp)[c(3, 4, 5, 338, 339, 340, 341, 342, 343, 344, 345)]))

write.csv(smartd_rplc, file = "smartd_rplc.csv", row.names = FALSE)
...
node.attr <- smartd_rplc[,c(1:18)] %>% 
  filter(., name %in% (c(cross_correlation_data3$from,
                         cross_correlation_data3$to)))
...
###there are 12 cross_subnetworks that with node bigger than 5.
##there are "11"  "10" "7" "13"  "28" "8" "33" "5" "25" "23" "35" "44"
cross_subnetwork11 <- 
  subgraph(graph = cross_correlation_data_igraph, 
           v = names(membership(cross_subnetwork))[membership(cross_subnetwork) == 11])

save(cross_subnetwork11, file = "cross_subnetwork11")


lay <- layout.auto(graph = cross_subnetwork11)
...
cross_subnetwork11_node_info <-
  vertex_attr(cross_subnetwork11) %>% 
  bind_cols()


write.csv(cross_subnetwork11_node_info,
          file = "cross_subnetwork11_node_info.csv",
          row.names = FALSE)

###cross_subnetwork11
cross_subnetwork11_ID_trans <-
  read.table(
    "cross_subnetwork11_ID_trans.txt",
    sep = ",",
    header = TRUE,
...
cross_subnetwork11_cross_temp <- 
cross_subnetwork11_node_info %>% 
  select(., name, Compound.name, HMDB.ID, KEGG.ID) %>% 
  left_join(., cross_subnetwork11_ID_trans, by = c("Compound.name" = "Query"))


# cross_subnetwork11_cross_temp <- 
cross_subnetwork11_cross_temp$KEGG[!is.na(cross_subnetwork11_cross_temp$KEGG) & cross_subnetwork11_cross_temp$KEGG == ''] <- NA

cross_subnetwork11_cross_temp$HMDB[!is.na(cross_subnetwork11_cross_temp$HMDB) & cross_subnetwork11_cross_temp$HMDB == ''] <- NA


cross_subnetwork11_cross_temp$KEGG

HMDB.KEGG <-
  apply(cross_subnetwork11_cross_temp, 1, function(x){
  HMDB1 <- x[3]
  KEGG1 <- x[4]
  HMDB2 <- x[6]
  KEGG2 <- x[8]
...
cbind(cross_subnetwork11_node_info$HMDB.ID, HMDB.KEGG[,1])

cross_subnetwork11_node_info[,c("HMDB.ID", 'KEGG.ID')] <-
  HMDB.KEGG



write.csv(cross_subnetwork11_node_info, 
          "cross_subnetwork11_node_info.csv", row.names = FALSE)

id <- cross_subnetwork11_node_info$KEGG.ID
id <- id[!is.na(id)]
...
save(cross_pathway11, file = "cross_pathway11")
...
ggplot(., aes(Overlap.frac * 100, -log(p.value, 10))) +
  ...
  ggrepel::geom_label_repel(aes(label = Pathway.name), 
                            data = cross_pathway11 %>% filter(., p.value < 0.05))
...
cross_betweenness11 <- betweenness(graph = cross_subnetwork11)
cross_degree11 <- igraph::degree(graph = cross_subnetwork11)
cross_closeness11 <- closeness(graph = cross_subnetwork11)

cross_importance11 <- 
  tibble(cross_betweenness11, cross_degree11, cross_closeness11) %>% 
  mutate_all(., function(x){
    (x - mean(x))/sd(x)
  }) %>% 
  mutate(.,mean = apply(., 1 ,mean)) %>% 
  mutate(peak.name = vertex_attr(graph = cross_subnetwork11, name = "name")) %>% 
  left_join(., cross_subnetwork11_node_info[,c("name", "Compound.name", "KEGG.ID")], 
            by = c('peak.name' = "name")) %>%  
...
cross_importance11[c(1:30),] %>% 
  ...
  scale_color_manual(values = c(
    "cross_betweenness11" = "#FFA319FF",
    "cross_closeness11" = "#8A9045FF",
    "cross_degree11" = "#C16622FF"
  )) +
  facet_grid(. ~ Class, 
             labeller = as_labeller(
               c("cross_betweenness11" = "Betweenness",
                 "cross_closeness11" = "Closeness",
                 "cross_degree11" = "Degree"
                 )
             )
             ) +
  labs(x = "", y = "") +
...
cross_importance11[1,]$peak.name
