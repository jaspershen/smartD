...
enrichPathway <- function(id,
  ...
  for (i in 1:ncol(all_matrix2)) {
    ...
    a3 <- length(sig_id) - a1# significant and not including pathway
    a4 <-
  ...
  info <-
    info %>% 
    mutate(Pathway.name = unlist(lapply(stringr::str_split(info$Pathway.name.ID, ";"), function(x) x[1])),
           Pathway.ID = unlist(lapply(stringr::str_split(info$Pathway.name.ID, ";"), function(x) x[2]))) %>% 
